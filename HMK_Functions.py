...
def effectiveWeeks(df):
    df_temp = df[(df.PeriodType!='Markdown')&(df.Wk_Key>=df.WeekFrom)&(df.Wk_Key<=df.WeekTo)&(df.StoreGrade>=df['SKU Store Grade'])] \
...
def grade_multiEquivalentU(df_inv, df_sku,var_MinTotSls):
    ...
    for grade in range(1,4):
        ...
        df_grade = df_inv[df_inv.StoreGrade>=grade].groupby('Option').agg({'STR Number':'count','SalesU':'sum'}).reset_index()
...
def grade_multiEquivalentV(df_inv, df_sku,var_MinTotSls):
    ...
    for grade in range(1,4):
        ...
        # Ros Per Grade calculation: Total sales and Amount of stores calculated for each Grade per Option
        df_grade = df_inv[df_inv.StoreGrade>=grade].groupby('Option').agg({'STR Number':'count','SalesV':'sum'}).reset_index()
...
def avgSellThru(df,df_sku,WeekMin,WeekMax,eDur,wDur,sDur):
    ...
    for grade in range(1,4):
        ...
        df_temp1 = df_table.groupby(['Option','Wk_Key','PeriodType']).agg({'SalesU':'sum','CSOHU':'sum','InStock':'sum',
        ...
        df_WeekStartTemp = df_temp2[df_temp2['InStock'] >= df_temp2['MinStores']].groupby('Option')['Wk_Key'].min().reset_index()
        df_WeekStartTemp.rename(columns={'Wk_Key':'WeekStartTemp'}, inplace=True)
        ...
        df_temp5 = df_temp4[((df_temp4['Wk_Key']>=df_temp4['WeekFrom'])&
                                        ...
                                        groupby(['Option','Wk_Key']).agg({'SalesU':'sum', 'CSOHU':'sum', 'InStock':'sum','MinStoresX':'mean'}).reset_index()
...
def ROS_ProdGrade(df_inv, df_sku, WeekMin,WeekMax,eDur,wDur,sDur,grade):
    ...
    df_final = df_table.groupby(['Option','SKU Store Grade']).agg({'SalesU':'sum','SalesV':'sum','InStock':'sum'}).reset_index()
    df_final.rename(columns={'SalesU':'Sales Units','SalesV':'Sales Value','InStock':'In-Stock Stores'}, inplace=True)
    ...
    df_temp2 = df_table[(df_table.PeriodType!='Markdown')&(df_table.StoreGrade>=df_table['SKU Store Grade'])].groupby(['Option','Wk_Key','PeriodType']) \
    .agg({'InStock':'sum'}).reset_index()
    ...
    df_final['ROS_ProdGrade'] = df_final['Sales Units in Period'] / df_final['In-Stock Stores in Period'] # MinInStock_X
    df_final['ROS_ProdGradeV'] = df_final['Sales Value in Period'] / df_final['In-Stock Stores in Period'] # MinInStock_X
...
def InventoryAggregation(df_inv, df_sku, WeekMin, WeekMax, eDur, wDur, sDur):
    # total
    df1 = df_inv.groupby(['Option','SKU Store Grade']).agg({'SalesU':'sum','SalesV':'sum','InStock':'sum'}).reset_index()
    ...
    ########################################################################
    # Stock
    df_temp = df_inv.loc[df_inv['Wk_Key']==WeekMax, ('Option','CSOHU')].groupby(['Option'])['CSOHU'].sum().reset_index()
    df_temp.rename(columns={'CSOHU':'Stock Units in Period'}, inplace=True)
    df1 = pd.merge(df1, df_temp, on='Option', how='left')

    ########################################################################
    # Adding average numbers from default table
    df_temp = df_inv.groupby('Option').agg({'Multi':'mean','GradeStores':'mean','MinStores':'mean','MinStoresX':'mean',
    ...
    ########################################################################
    # PromoFactor
    ROS_PerGrade_Promo = df1[df1['SKU Store Grade']==1]['Sales Units in Period Promo'].sum() / df1[df1['SKU Store Grade']==1]['In-Stock Stores in Period Promo'].sum()
    ROS_PerGrade_Regular = df1[df1['SKU Store Grade']==1]['Sales Units in Period Regular'].sum() / df1[df1['SKU Store Grade']==1]['In-Stock Stores in Period Regular'].sum()
    ...
    df1['Promo Adjusted ROS'] = np.where(df1['Sales Units in Period Promo']==0,
                                         df1['ROS_ProdGrade'],
                                         (df1['Sales Units in Period Promo'] / PromoFactor + df1['Sales Units in Period Regular']) / df1['In-Stock Stores in Period'])
    df1['Promo Adjusted ROS V'] = np.where(df1['Sales Value in Period Promo']==0,
                                           df1['ROS_ProdGradeV'],
                                           (df1['Sales Value in Period Promo'] / PromoFactor + df1['Sales Value in Period Regular']) / df1['In-Stock Stores in Period'])
...
def calcScoringSellThru(df_score,ST_Tier_1,ST_Tier_2):
    ...
    colnames = ['Option','SKU Merch Type','MerchGroup','Avg Sell-Through In Period','SKU Sub Department','ItemCountMer']
    ...
    for merch in list(df_score['MerchGroup'].unique()):
        ...
        df_temp['ST Value'] = np.where(df_temp['RankX']<=np.ceil(ST_Tier_1*df_temp['ItemCountMer']),'Tier1_V',
                                 np.where(df_temp['RankX']<=np.ceil(ST_Tier_2*df_temp['ItemCountMer']),'Tier2_V','-'))
...
def calcScoringROS(score_variable,score_valueA,score_valueB,df_score,hier):
    ...
    if hier[2]=='2':
        ...
        for dep in list(df_score['SKU Sub Department'].unique()):    
            df_temp = df_score[df_score.ItemExcl==0].loc[df_score['SKU Sub Department']==dep,colnames].sort_values(score_variable, ascending=False).copy()
            df_temp['RankX'] = np.arange(df_temp.shape[0])+1
            #df_temp['Cumm%'] = round(df_temp['RankX']/df_temp['RankX'].max(),2)
            df_temp[score_valueA] = np.where(df_temp['RankX']<=np.ceil(df_temp['Tier1']*df_temp['ItemCountDep']),'Tier1_V',
                                     np.where(df_temp['RankX']<=np.ceil(df_temp['Tier2']*df_temp['ItemCountDep']),'Tier2_V',
                                             np.where(df_temp['RankX']<=np.ceil(df_temp['Tier3']*df_temp['ItemCountDep']),'Tier3_V','-')))
        ...
        df = pd.merge(df_score[['Option','SKU Sub Department',score_variable]],df2,on='SKU Sub Department',how='inner')
        ...
        for dep in list(df_score['SKU Sub Department'].unique()):    
            ...
            df_temp['RankX'] = np.arange(df_temp.shape[0])+1
            #df_temp['Cumm%'] = round(df_temp['RankX']/df_temp['RankX'].max(),2)
            df_temp[score_valueA] = np.where(df_temp['RankX']<=np.ceil(df_temp['SlsTier1']*df_temp['ItemCountDep']),'Tier1_V',
                                     np.where(df_temp['RankX']<=np.ceil(df_temp['SlsTier2']*df_temp['ItemCountDep']),'Tier2_V','-'))
...
def calcScoringMD(df_score,MD_Tier1,MD_Tier2):
    ...
    colnames = ['Option','SKU Merch Type','MerchGroup','Sales Value','Markdown Value','SKU Sub Department','ItemCountMer']
    df1 = pd.DataFrame(columns=colnames)

    for merch in list(df_score['MerchGroup'].unique()):
        df_temp = df_score[df_score.ItemExcl==0].loc[df_score['MerchGroup']==merch,colnames] \
        .sort_values(by='MerchGroup', ascending=False).copy()
        df_temp['MD_SLS'] = np.where(df_temp['Markdown Value']>0,np.where(df_temp['Sales Value']>0,df_temp['Markdown Value']/df_temp['Sales Value'],1),0)
